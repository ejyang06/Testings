...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor = g.conn.execute("SELECT loginID,pw,userID FROM Hikers")
        loginID = []
        password = []
        userID = []
	    ...
	    userID.append(result[2])
        cursor.close()

	indexNum = loginID.index(int(request.form['username']))
        ...
        elif (str(request.form['password']) != password[indexNum].encode('utf8')) or str(request.form['password']) == "":
            error = 'Invalid password'
        else:
            session['logged_in'] = True
	    global nowuserID, nowloginID
	    nowuserID = userID[indexNum]
	    nowloginID = loginID[indexNum]
